...
class JiraConfig:
    ...
    def from_env(cls) -> "JiraConfig":
        ...
        if oauth_config:
            ...
            logger.info("JIRA configured for pass-through authentication only - all credentials must come from HTTP headers")
        else:
            if is_cloud:
                error_msg = "Cloud authentication requires JIRA_USERNAME and JIRA_API_TOKEN, JIRA_PERSONAL_TOKEN, OAuth configuration, or ATLASSIAN_PASS_THROUGH_ONLY=true for header-based authentication"
            else:
                error_msg = "Server/Data Center authentication requires JIRA_PERSONAL_TOKEN, JIRA_USERNAME and JIRA_API_TOKEN, OAuth configuration, or ATLASSIAN_PASS_THROUGH_ONLY=true for header-based authentication"
            raise ValueError(error_msg)
